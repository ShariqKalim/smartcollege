import React from "react";

const Sem_6_ece = () => {
  return (
    <>
      <div className="w-10/12 justify-between flex flex-wrap mx-auto">
        <a href="" className="no-underline">
          <div className="res_1 w-96 border-4 p-10  h-52 grid place-items-center mb-10 mt-24 shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">
              Control System and Instrumentation
            </h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_1 w-96 border-4 p-10  h-52 grid place-items-center mb-10 mt-24 shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Computer Network</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_1 w-96 border-4 p-10  h-52 grid place-items-center mb-10 mt-24 shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Economics for Engineers</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Introduction to MEMS</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Bio-Medical Electronics</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">CMOS VLSI Design</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10 mt-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Information Theory and Coding</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10 mt-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">
              Electronic Measurement & Measuring Instruments
            </h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10 mt-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Operating System</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10 mt-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Object Oriented Programming</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10 mt-10  h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">Universal Human Values</h4>
          </div>
        </a>
        <a href="" className="no-underline">
          <div className="res_2 w-96 border-4 p-10 mt-10 mb-3 h-52 grid place-items-center  shadow-xl text-center transform hover:scale-110">
            <h5 className="w-full bg-gradient-to-r from-blue-500 to-blue-400 absolute -mt-40 text-white font-medium p-1">
              Classnotes
            </h5>
            <h4 className="text-gray-800">
              Mini Project/ Electronic Design Workshop
            </h4>
          </div>
        </a>
      </div>
    </>
  );
};

export default Sem_6_ece;
